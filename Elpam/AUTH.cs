...
namespace Elpam
{
    ...
    internal class AUTH
    {
        ...
        private static void AddHWID(string user, string hwid, bool role)
        {
            User add = new User()
            {
            ...
            var set = client.Set(@"USERS/" + user + "/" + hwid, add);
        }

        static bool flagAuth = true;
        public static void Authen()
        {
            if (flagAuth)
            {
                ...
                string hwid = Value();
                var resSet = client.Get(@"SETTINGS");
                SetAUTH getSetAUTH = resSet.ResultAs<SetAUTH>();
                if (getSetAUTH.Status == true && getSetAUTH.Version == version)
                {
                    var resInfo = client.Get(@"USERS/" + userName);
                    Dictionary<string, User> user = resInfo.ResultAs<Dictionary<string, User>>();

                    if (resInfo.Body != "null") //username not exist
                    {
                        foreach (var info in user)
                        {
                            if (info.Key == hwid) // hwid exist
                            {
                                if (info.Value.Role)
                                {
                                    //update time
                        ...
                        if (getSetAUTH.Receive)
                        {
                            AddHWID(userName, hwid, getSetAUTH.RoleReceive);
                            flagAuth = true;
                            return;
                        }
                    }
